    ...
    <div className="card">
      <div className="card-header flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-200">Disrupted Routes</h3>
          <span className="badge badge-critical text-xs">{disrupted_routes_count} blocked</span>
        </div>
        <span className="text-xs text-slate-500">All routes via {disruptedLocation}</span>
      </div>
      <div className="card-body">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
          {disruptedRouteIds.slice(0, 20).map(routeId => (
            <div
              key={routeId}
              className="flex items-center gap-2 bg-red-900/15 border border-red-800/30 rounded-lg px-3 py-2"
            >
              <AlertTriangle className="w-3 h-3 text-red-400 flex-shrink-0" />
              <span className="text-xs text-slate-300 font-mono truncate" title={routeId}>
                {routeToLabel(routeId)}
              </span>
              <span className="text-xs text-red-400 ml-auto flex-shrink-0">BLOCKED</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-3 flex items-start gap-2 text-xs text-slate-400 bg-slate-800/40 rounded-lg p-3">
